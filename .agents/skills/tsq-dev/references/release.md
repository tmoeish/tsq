...
- 维护旧大版本（**唯一的长期分支例外**）：基于最后一个该版本的 tag 切分支
  ...
  在上面修、在上面打 tag，发版时加 `--allow-branch`。v4 的这条线是 `v4`（从 `v4.10.0` 切出）：
  `.github/workflows/go.yml` 的 `push` / `pull_request` 都列着 `v4`，否则指向它的 PR 一个检查
  都不跑；`latest_tag()` 只比同一主版本的 tag，否则 v5 一发布这条线就再也过不了 `release-check`。
  修复先落在 `v4` 上，同一个问题 v5 也有的，另开 PR 修 `main`——两条线的代码已经分开，不做合并。
