...
- 维护旧大版本（**唯一的长期分支例外**）：基于最后一个该版本的 tag 切分支
  ...
  在上面修、在上面打 tag，发版时加 `--allow-branch`。v4 的这条线是 `v4`（2026-09-22 从 `v4.10.0`
  切出，CI 触发与 `latest_tag()` 的调整在那条分支自己的 `release.md` 里）。修复先落在 `v4` 上，
  v5 也有的问题另开 PR 修 `main`——两条线的代码已经分开，**不做合并**。
