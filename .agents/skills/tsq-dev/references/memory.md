...
65535 注释"最紧的"，SQLite 其实是 32766——**修一类 bug 要把这一类的实例都数一遍**。
...
- **v5 明确不支持复合主键**（维护者定案）：`pk=A,B` 报错并指向单列代理键 + `//tsq:unique A,B`；要支持
  就是 v6（主键字段、`FetchXxxByID`、`BatchDeleteByPK`、乐观锁 WHERE 全要变形状）。
...
包级初始化顺序只认初始化表达式里写出来的引用。v4 经接口方法取列切片，分析看不见，查询可能先于切片
初始化（元素全 `nil`），**炸不炸取决于文件名顺序**。
