...
### 决定：方言能力从**渲染好的 SQL**读，但要跳过字面量 (2026-09-16，v5)

此前记着"正解是从 `querySpec` 导出"。**那条是错的**：子查询以 SQL 文本形式进入外层条件，只看
builder 结构会**漏掉**子查询里的 `FULL JOIN` 或 CTE；漏报比误报糟——漏了直接去数据库上炸，而这道
检查的意义恰恰是把"这个方言做不到"变成带能力名和方言名的错误。真正的 bug 是裸 `strings.Contains`
不区分关键字和字面量，`Where(Note.EQVal(" FOR UPDATE "))` 在 SQLite 上因此被**拒绝执行**；
`walkSQL` 一直会跳过字面量和注释，只是这处没用它。**一道用来把失败变清楚的检查，误报时就是在制造
它本来要消灭的那种失败。**
...
- 生成的批量取数叫 `FetchXxxByYyy`，缺行时包装 `sql.ErrNoRows`；生成代码的错误**只带主键**，不序列化
  整行（列值会进日志）。Result 和表一样用 `Xxx__Cols`，不生成描述符变量。
