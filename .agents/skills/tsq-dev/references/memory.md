...
按主题归入已有小节，不要按日期另起一条；只有日期本身是信息时才写它。就地纠正错误的条目并删除陈旧的条目，不要追加自相矛盾的内容——过时的内存比
没有内存更糟，因为人会相信它。
...
操作要写显式 `And()`——"静默去掉过滤条件"在写路径同样不允许。`updated_at` / `deleted_at`
库层不碰：`Table` 接口只暴露 `VersionColumn()`，往接口加方法会让手写 `Table` 实现编不过；
生成器出按条件软删的包装等有真实用量再说。

## 构建与代码生成

### 生成的表变量必须对列切片留下一次可见的引用 (2026-09-03)

`Cols()` 是**通过接口方法在运行期**去取 `Xxx__Cols` 的，而 Go 的包级初始化顺序只认初始化
表达式里写出来的引用。只选部分列的**投影**查询变量从头到尾不提那个切片，于是可以排在它
前面初始化。那时切片**不是空的**（空会被当成"没有列信息"放行），而是长度已满、元素全
`nil`——切片头是编译期静态数据，元素赋值发生在 init 里。于是报出 `column X does not belong
to table Y`：指着一列明明存在的列说它不属于明明就是它的表。**炸不炸取决于包内文件名顺序**，
同一种写法在一个文件里正常、换个文件就 panic，照着能跑的那个抄的人换张表就炸。
...
**"最新 tag"要按主版本线算**（2026-09-22，切 `v4` 维护分支时）：`latest_tag()` 原来取全仓最新，
v5 的 tag 一打出来，本分支每个 v4 补丁都会被判成倒退、`release.py` 也会以 v5 为基线算版本号。
现在只比 `module_major()` 那条线。另：CI 的 gosec 曾只在门禁那一遍排除 G201/G304，上传代码扫描的
SARIF 那一遍不排除，于是门禁已接受的每处读文件都开一条告警——**两遍用同一组排除**。
