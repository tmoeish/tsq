...
- `.agents/skills/tsq-dev` 包含**开发本仓的工程上下文** —— `architecture.md`、
  `feature-map.md`、`codegen.md`、`change-impact.md`（索引，条目在 `impact/<域>.md`）、
  `release.md`，以及 `references/memory/<主题>.md` 里带日期的事故与决策（`memory.md` 是判据与索引）。
...
- **可空性在类型上**：可空字段是 `NullColumn[O, T]`，按值类型比较，NULL 只用 `SetNull` 写；可能为
  NULL 的值读进不能存 NULL 的字段，在读行之前就报错。不要回到"`Column[O, sql.NullString]` 靠运行期
  扫描失败兜底"，也不要把这道检查挪进 `Build`（会拒掉合法的子查询和 CTE）。
...
- 注解是 `//tsq:` 指令行（`//go:` 那种形态），gofmt 不碰它们，**没有 fmt 这一步**。
...
- 谓词命名的分工：`Op(rhs)` 是唯一入口，右值是列、参数、子查询或 `tsq.Val(v)`（列表是
  `tsq.Vals(vs...)`），否定一律 `Not*`。**不要加回 `OpVal(v)` 方法**（理由见 `memory/query.md`）；
...
- 跟随仓库现有风格，改动保持外科手术式。
- **优先使用当前 Go 版本的新特性**（`go.mod` 现在是 1.27），不要为了"和旧代码长得一样"
  退回旧写法：泛型方法（`func (r *T) M[P any](...)`）、`reflect.TypeAssert`、`errors.AsType`、
  `slices` / `maps` 包、`min` / `max` 内建、`for range n`。判据是可读性和类型安全，不是新旧。
  升级 Go 版本时顺手过一遍：新版本有哪些特性能把现有的绕路写法收掉。
...
- 改代码**之前**，读覆盖你要碰的领域的上下文文件，并对每个匹配的触发器处理
  `.agents/skills/tsq-dev/references/change-impact.md`：它是索引，按速查挑出匹配的
  `impact/<域>.md` 读，不要整域加载。耦合的工作在同一波里做完。
...
- 一个被修两次的问题是一个不彻底的修复：直到 `impact/*.md` 或 `memory/*.md` 阻止了第三次
  发生，第二次修复才算完成。
- 就地更正，删掉变错的条目。**永远不要追加自相矛盾的内容**——过时的上下文文件比缺失的更糟，
  因为人会相信它。
- `change-impact.md`、`memory.md` 是索引：新写一条触发器要在速查里补一行标题，新开一份主题
  文件要进索引的路由表。`make skill-check` **无条件**比对索引和子文件，这一半不提供豁免——
  漏掉的那一行等于那条耦合不存在，因为读的人以为自己看过了全部。
- `make skill-check` 还把最容易忘的几条耦合钉死了。确实判断过不需要动技能时，用
  `SKIP_SKILL_CHECK=<触发器名>` 逐条豁免，并在提交正文里写理由。无理由的总开关不提供。

## 项目内存

`.agents/skills/tsq-dev/references/memory.md`（判据与索引）和 `references/memory/<主题>.md`
（条目）被提交，所以每台机器和 agent 共享一份记忆。和它描述的代码在同一波里更新它；新记录写进
对应主题的那一份，不写回索引。
...
- 不要复述规则、包布局或使用者契约——本文件、`architecture.md` 和 `skills/tsq` 各自拥有
  它们。
- 按主题归入已有小节，不要按日期另起一条；只有日期本身是信息时才写它。就地纠正错误的条目并删除陈旧的条目；不要追加矛盾的内容。
- **这些文件是每个 agent 的上下文来源，长度是每次会话都要付的成本，所以它们必须能缩。**
  按主题拆开是为了让一次会话只读一两份，不是为了让总量随便涨；写满了先合并同主题的条目。
...
- 删除不是销毁：文件被提交，`git log -p --follow` 能还原任何被删的条目。删除的代价是
  "不再默认出现在上下文里"，不是"永远失去"。
- `make memory-check` 在未提交的功能性改动没有携带内存更新时失败，并**无条件**守着索引加
  全部主题文件的总行数上限。发版波（只改 `internal/buildinfo/buildinfo.go`）被精确豁免。如果一波确实没教会
...
- **"攒着"分两种，归宿不同，不要混。** `## [未发布]` 段只收**已经做完的改动**。
  **发现了但决定暂时不做的问题**不属于那里——CHANGELOG 描述已发生的变化，不是待办清单。
  它的归宿是 `memory/` 里对应主题的一条带日期的记录（写清现象、为什么现在不值得动、什么条件下
