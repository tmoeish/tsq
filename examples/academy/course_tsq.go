...
func init() {
	tsq.RegisterTable(
		...
		tsq.TableIndex{Name: "ux_course_title", Unique: true, Fields: []string{"title"}},
		// Declared non-unique indexes.
		tsq.TableIndex{Name: "idx_course_instructor_id", Fields: []string{"instructor_id"}},
		tsq.TableIndex{Name: "idx_course_prerequisite_id", Fields: []string{"prerequisite_id"}},
		tsq.TableIndex{Name: "idx_course_track_id", Fields: []string{"track_id"}},
	)
}
...
func ListCourseByIDInOrErr(
	...
	ordered, missing := tsq.MatchByInputOrder(iDs, list, func(row *Course) int64 {
		return row.ID
	})
	if len(missing) > 0 {
