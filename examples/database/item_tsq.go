// Code generated by tsq-v3.7.1. DO NOT EDIT.

package database

import (
	"context"
	tsqsql "database/sql"
	tsqtime "time"

	null "gopkg.in/nullbio/null.v6"

	"github.com/juju/errors"
	"github.com/tmoeish/tsq"
)

// =============================================================================
// Table Interface Implementation
// =============================================================================

// TableItem implements the tsq.Table interface for Item.
var TableItem tsq.Table = Item{}

// TSQOwner marks Item as a TSQ owner.
func (i Item) TSQOwner() {}

// Table returns the database table name for Item.
func (i Item) Table() string { return "item" }

// Cols returns all generated columns for Item.
func (i Item) Cols() []tsq.SQLColumn {
	return tsq.SQLColumns(TableItemCols...)
}

// SearchColumns returns columns that support keyword search for Item.
func (i Item) SearchColumns() []tsq.SearchColumn {
	return []tsq.SearchColumn{
		Item_Name,
	}
}

// PrimaryKeys returns the primary key columns for Item.
func (i Item) PrimaryKeys() []string {
	return []string{"id"}
}

// AutoIncrement reports whether Item uses an auto-increment primary key.
func (i Item) AutoIncrement() bool { return true }

// VersionColumn returns the optimistic-lock version column for Item, if any.
func (i Item) VersionColumn() string {
	return ""
}

// Column definitions for Item table.
var (
	Item_CategoryID = tsq.NewCol("category_id", "CategoryID", func(t *Item) *int64 { return &t.CategoryID })
	Item_CreatedAt  = tsq.NewCol("created_at", "created_at", func(t *Item) *null.Time { return &t.CreatedAt })
	Item_ID         = tsq.NewCol("id", "id", func(t *Item) *int64 { return &t.ID })
	Item_Name       = tsq.NewCol("name", "Name", func(t *Item) *string { return &t.Name })
	Item_Price      = tsq.NewCol("price", "Price", func(t *Item) *int64 { return &t.Price })
)

// TableItemCols is the list of all selectable columns for Item table.
var TableItemCols = []tsq.BoundColumn[Item]{
	Item_CategoryID,
	Item_CreatedAt,
	Item_ID,
	Item_Name,
	Item_Price,
}

// =============================================================================
// Table Registration and Index Initialization
// =============================================================================

func init() {
	tsq.RegisterTable(
		TableItem,
		func(db *tsq.Engine) error {
			// Upsert unique indexes.
			if err := tsq.UpsertIndex(db, "item", true, "ux_item_name", []string{"name"}); err != nil {
				return errors.Annotate(err, "upsert ux_item_name@item")
			}
			// Upsert non-unique indexes.
			if err := tsq.UpsertIndex(db, "item", false, "idx_category", []string{"category_id"}); err != nil {
				return errors.Annotate(err, "upsert idx_category@item")
			}
			return nil
		},
	)
}

// =============================================================================
// Query by Primary Key
// =============================================================================
var getItemByIDQuery *tsq.Query[Item]

func init() {
	var err error
	getItemByIDQuery, err = tsq.
		Select(TableItemCols...).
		From(TableItem).
		Where(Item_ID.EQVar()).
		Build()
	if err != nil {
		panic(errors.Annotate(err, "initialize getItemByIDQuery"))
	}
}

// GetItemByID retrieves a Item record by its primary key.
// Returns (nil, nil) if the record is not found.
func GetItemByID(
	ctx context.Context,
	db tsq.SQLExecutor,
	iD int64,
) (*Item, error) {
	row := &Item{}
	err := getItemByIDQuery.Load(ctx, db, row, iD)
	if err != nil {
		if errors.Is(err, tsqsql.ErrNoRows) {
			return nil, nil
		}
		return nil, errors.Trace(err)
	}
	return row, nil
}

// GetItemByIDOrErr retrieves a Item record by its primary key.
// Returns (nil, database/sql.ErrNoRows) if the record is not found.
func GetItemByIDOrErr(
	ctx context.Context,
	db tsq.SQLExecutor,
	iD int64,
) (*Item, error) {
	row := &Item{}
	err := getItemByIDQuery.Load(ctx, db, row, iD)
	if err != nil {
		return nil, errors.Trace(err)
	}
	return row, nil
}

// ListItemByIDIn retrieves multiple Item records by a set of primary key values.
// Records not found are silently ignored.
func ListItemByIDIn(
	ctx context.Context,
	db tsq.SQLExecutor,
	iDs ...int64,
) ([]*Item, error) {
	query, err := tsq.
		Select(TableItemCols...).
		From(TableItem).
		Where(Item_ID.In(iDs...)).
		Build()
	if err != nil {
		return nil, errors.Annotate(err, "build query")
	}
	return tsq.List(ctx, db, query)
}

// ListItemByIDInOrErr retrieves multiple Item records by a set of primary key values.
// Returns an error if any of the specified records are not found.
func ListItemByIDInOrErr(
	ctx context.Context,
	db tsq.SQLExecutor,
	iDs ...int64,
) ([]*Item, error) {
	query, err := tsq.
		Select(TableItemCols...).
		From(TableItem).
		Where(Item_ID.In(iDs...)).
		Build()
	if err != nil {
		return nil, errors.Annotate(err, "build query")
	}

	list, err := tsq.List(ctx, db, query)
	if err != nil {
		return nil, errors.Trace(err)
	}

	match := tsq.MatchByInputOrder(iDs, list, func(row *Item) int64 {
		return row.ID
	})
	if len(match.Missing) > 0 {
		return nil, errors.Errorf("Item(s) not found: %v", match.Missing)
	}
	return match.Ordered, nil
}

// =============================================================================
// Query by Unique Indexes
// =============================================================================
var getItemByNameQuery *tsq.Query[Item]

func init() {
	var err error
	getItemByNameQuery, err = tsq.
		Select(TableItemCols...).
		From(TableItem).
		Search(TableItem.SearchColumns()...).
		Where(
			Item_Name.EQVar(),
		).
		Build()
	if err != nil {
		panic(errors.Annotate(err, "initialize getItemByNameQuery"))
	}
}

// GetItemByName retrieves a Item record by unique index ux_item_name.
// Returns (nil, nil) if the record is not found.
func GetItemByName(
	ctx context.Context,
	db tsq.SQLExecutor,
	name string,
) (*Item, error) {
	row := &Item{}
	err := getItemByNameQuery.Load(
		ctx, db, row,
		name,
	)
	if err != nil {
		if errors.Is(err, tsqsql.ErrNoRows) {
			return nil, nil
		}
		return nil, errors.Trace(err)
	}
	return row, nil
}

// GetItemByNameOrErr retrieves a Item record by unique index ux_item_name.
// Returns (nil, database/sql.ErrNoRows) if the record is not found.
func GetItemByNameOrErr(
	ctx context.Context,
	db tsq.SQLExecutor,
	name string,
) (*Item, error) {
	row := &Item{}
	err := getItemByNameQuery.Load(
		ctx, db, row,
		name,
	)
	if err != nil {
		return nil, errors.Trace(err)
	}
	return row, nil
}

// ExistsItemByName checks if a Item record exists by unique index ux_item_name.
func ExistsItemByName(
	ctx context.Context,
	db tsq.SQLExecutor,
	name string,
) (bool, error) {
	rs, err := getItemByNameQuery.Exists(
		ctx, db,
		name,
	)
	return rs, errors.Trace(err)
}

// =============================================================================
// Query by Indexes
// =============================================================================
var ListItemByCategoryIDQuery *tsq.Query[Item]

func init() {
	var err error
	ListItemByCategoryIDQuery, err = tsq.
		Select(TableItemCols...).
		From(TableItem).
		Search(TableItem.SearchColumns()...).
		Where(
			Item_CategoryID.EQVar(),
		).
		Build()
	if err != nil {
		panic(errors.Annotate(err, "initialize ListItemByCategoryIDQuery"))
	}
}

// CountItemByCategoryID returns the count of Item records matching index idx_category.
func CountItemByCategoryID(
	ctx context.Context,
	db tsq.SQLExecutor,
	categoryID int64,
) (int, error) {
	rs, err := ListItemByCategoryIDQuery.Count(
		ctx, db,
		categoryID,
	)
	if err != nil {
		return 0, errors.Annotate(err, "query by index idx_category")
	}
	return rs, nil
}

// ListItemByCategoryID retrieves Item records by index idx_category.
func ListItemByCategoryID(
	ctx context.Context,
	db tsq.SQLExecutor,
	categoryID int64,
) ([]*Item, error) {
	data, err := tsq.List(
		ctx, db, ListItemByCategoryIDQuery,
		categoryID,
	)
	if err != nil {
		return nil, errors.Annotate(err, "query by index idx_category")
	}
	return data, nil
}

// PageItemByCategoryID retrieves Item records by index idx_category with pagination.
func PageItemByCategoryID(
	ctx context.Context,
	db tsq.SQLExecutor,
	page *tsq.PageReq,
	categoryID int64,
) (*tsq.PageResp[Item], error) {
	rs, err := tsq.Page(
		ctx, db, page, ListItemByCategoryIDQuery,
		categoryID,
	)
	if err != nil {
		return nil, errors.Annotate(err, "query by index idx_category")
	}
	return rs, nil
}

var ListItemByCategoryIDInQuery *tsq.Query[Item]

func init() {
	var err error
	ListItemByCategoryIDInQuery, err = tsq.
		Select(TableItemCols...).
		From(TableItem).
		Where(
			Item_CategoryID.InVar(),
		).
		Build()
	if err != nil {
		panic(errors.Annotate(err, "initialize ListItemByCategoryIDInQuery"))
	}
}

// ListItemByCategoryIDIn retrieves multiple Item records by index idx_category using an IN clause.
func ListItemByCategoryIDIn(
	ctx context.Context,
	db tsq.SQLExecutor,
	categoryIDs ...int64,
) ([]*Item, error) {
	list, err := tsq.List(
		ctx, db, ListItemByCategoryIDInQuery,
		categoryIDs,
	)
	if err != nil {
		return nil, errors.Annotate(err, "query by index idx_category")
	}
	return list, nil
}

// =============================================================================
// List All Records
// =============================================================================
var listItemQuery *tsq.Query[Item]

func init() {
	var err error
	listItemQuery, err = tsq.
		Select(TableItemCols...).
		From(TableItem).
		Search(TableItem.SearchColumns()...).
		Build()
	if err != nil {
		panic(errors.Annotate(err, "initialize listItemQuery"))
	}
}

// CountItem returns the total count of Item records.
func CountItem(
	ctx context.Context,
	tx tsq.SQLExecutor,
) (int, error) {
	return listItemQuery.Count(ctx, tx)
}

// ListItem retrieves all Item records.
func ListItem(
	ctx context.Context,
	tx tsq.SQLExecutor,
) ([]*Item, error) {
	return tsq.List(ctx, tx, listItemQuery)
}

// PageItem retrieves Item records with pagination.
func PageItem(
	ctx context.Context,
	tx tsq.SQLExecutor,
	page *tsq.PageReq,
) (*tsq.PageResp[Item], error) {
	return tsq.Page(ctx, tx, page, listItemQuery)
}

// =============================================================================
// CRUD Operations
// =============================================================================

// Insert inserts a new Item record.
func (i *Item) Insert(
	ctx context.Context,
	db tsq.SQLExecutor,
) error {
	i.CreatedAt = null.TimeFrom(tsqtime.Now())
	err := tsq.Insert(ctx, db, i)
	if err != nil {
		return errors.Annotatef(err, "insert Item: %s", tsq.CompactJSON(i))
	}
	return nil
}

// Update updates an existing Item record.
func (i *Item) Update(
	ctx context.Context,
	db tsq.SQLExecutor,
) error {
	err := tsq.Update(ctx, db, i)
	if err != nil {
		return errors.Annotatef(err, "update Item: %s", tsq.CompactJSON(i))
	}
	return nil
}

// Delete permanently removes a Item record.
func (i *Item) Delete(
	ctx context.Context,
	db tsq.SQLExecutor,
) error {
	err := tsq.Delete(ctx, db, i)
	if err != nil {
		return errors.Annotatef(err, "delete Item: %s", tsq.CompactJSON(i))
	}
	return nil
}

// =============================================================================
// Custom Query Helpers
// =============================================================================
// ListItemByQuery executes a custom query to retrieve a list of Item records.
func ListItemByQuery(
	ctx context.Context,
	tx tsq.SQLExecutor,
	qb *tsq.Query[Item],
	args ...any,
) ([]*Item, error) {
	return tsq.List(ctx, tx, qb, args...)
}

// PageItemByQuery executes a custom query to retrieve a page of Item records.
func PageItemByQuery(
	ctx context.Context,
	tx tsq.SQLExecutor,
	page *tsq.PageReq,
	qb *tsq.Query[Item],
	args ...any,
) (*tsq.PageResp[Item], error) {
	return tsq.Page(ctx, tx, page, qb, args...)
}
