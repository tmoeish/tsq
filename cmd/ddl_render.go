...
func buildDDLArtifacts(packagePath string, list []*tsq.StructInfo, outDir string) (ddlArtifacts, error) {
	...
	for _, item := range list {
		if item == nil || item.TableMeta == nil || item.IsResult || len(item.Fields) == 0 {
			continue
		}
...
func renderDDLIndexStatements(table *tsq.StructInfo, dialect ddlDialectSpec) ([]string, error) {
	...
	for _, item := range []struct {
		...
		for _, idx := range item.items {
			...
			statements = append(statements, dialect.dialect.DDLCreateIndex(table.Table, idx.Name, quotedFields, item.unique))
		}
	}
...
func ddlDialectName(dialect ddlDialectSpec) string {
