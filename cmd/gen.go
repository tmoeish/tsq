...
Troubleshooting:
  ...
  - generator validation errors point to the offending struct or field`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if v {
			_, _ = fmt.Fprintf(os.Stderr, "parsed %d table(s), %d result(s)\n", stats.Tables, stats.Results)
		}
		...
		if dryRunFlag {
			printGenerationPlan(cmd.OutOrStdout(), plan)

			if v {
				printGenerationSummary(os.Stderr, plan)
			}
		...
		if checkFlag {
			...
			if err := ensureGenerationPlanUpToDate(plan); err != nil {
				return errors.Trace(err)
			}
