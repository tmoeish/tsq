...
var GenCmd = &Command{
	Name:  "gen",
	Usage: "gen <package-or-dir> [flags]",
...
Troubleshooting:
  ...
  - generator validation errors point to the offending struct or field`,
	...
	flags: func(fs *flag.FlagSet) {
		...
		fs.BoolVar(&checkFlag, "check", false, "render in memory and fail if generated files are out of date")
		fs.BoolVar(&v, "verbose", false, "print each generated file path")
		fs.BoolVar(&v, "v", false, "shorthand for --verbose")
	},
	run: func(cmd *Command, args []string) error {
		if err := exactOnePackageArg(args); err != nil {
			return err
		}
...
func exactOnePackageArg(args []string) error {
	if len(args) == 1 {
		return nil
	}

	return fmt.Errorf("tsq gen expects exactly one package path, got %d", len(args))
}
