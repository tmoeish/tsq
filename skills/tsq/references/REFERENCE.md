...
- `Update`, `Delete`, `HardDelete` the same way. `Update` writes every column TSQ may write, so
  **a row read with a partial `Select` must name what it saves**: `TableCourse.Update(ctx, db, &row,
  TableCourse.Title)` / `row.Update(ctx, db, TableCourse.Title)` writes only `title` (plus
  `updated_at` and `version`), where a plain `Update` would overwrite the unread columns with zero
  values
