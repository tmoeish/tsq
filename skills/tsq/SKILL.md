...
- `query.Page` takes a typed `tsq.Paging`; convert an HTTP `tsq.PageRequest` with `req.Paging(sortableCols...)`, which is also where it is validated. A size above the runtime cap is served capped, not rejected.
