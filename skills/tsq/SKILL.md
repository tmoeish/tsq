...
- Functions of a column return `tsq.Expression[T]`, which `Select` does not take: project it with `tsq.MapInto`, or read it alone with `tsq.SelectValue` (`tsq.SelectNullValue` when it can be NULL), whose rows are the values. `query.AsSubquery(expr)` / `tsq.BuildSubquery(stage, expr)` make a typed subquery.
