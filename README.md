...
- `Runtime`：注册表、索引声明、tracer，以及默认数据库上下文
...
| **怎么跑第一条查询？** | `tsq gen ./db` → `tsq.Init(...)` → `exec := tsq.DefaultRuntime().Executor()` → `tsq.Select(...).From(table).Where(...).Build()` → `tsq.List(...)`。 |
...
- 传普通 `*sql.DB` / `*tsq.Runtime`：允许按 chunk 执行，前面成功的 chunk 不会因为后面失败自动回滚
- 传 `*sql.Tx`（或通过 `WrapExecutor` 带上 runtime 方言）：整个 chunked 操作就运行在该事务里
