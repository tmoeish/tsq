...
func (t *strictMockTable) ManagedColumns() ManagedColumns { return ManagedColumns{} }
...
func TestJoinValidation_SameTableWithoutAliasRejectsJoin(t *testing.T) {
	...
	userID := newColForTable[Table, int](users, "id", "id", nil)
	parentID := newColForTable[Table, int](users, "parent_id", "parent_id", nil)
...
func TestJoinValidation_RejectsJoinConditionWithoutIntroducedTable(t *testing.T) {
	...
	orders := newMockTable("orders")
	userID := newColForTable[Table, int](users, "id", "id", nil)
	orderStatus := newColForTable[Table, int](orders, "status", "status", nil)
...
func TestJoinValidation_RejectsJoinConditionReferencingFutureTable(t *testing.T) {
	...
	userID := newColForTable[Table, int](users, "id", "id", nil)
	orderUserID := newColForTable[Table, int](orders, "user_id", "user_id", nil)
	itemOrderID := newColForTable[Table, int](items, "order_id", "order_id", nil)
...
func TestJoinValidation_SelfJoinWithAliasSucceeds(t *testing.T) {
	...
	userID := newColForTable[Table, int](users, "id", "id", nil)
	userParentID := newColForTable[Table, int](users, "parent_id", "parent_id", nil)
	parentID := newColForTable[Table, int](parentUsers, "id", "id", nil)
...
func TestJoinValidation_NonEqualityConditionSucceeds(t *testing.T) {
	...
	userScore := newColForTable[Table, int](users, "score", "score", nil)
	orderMinimum := newColForTable[Table, int](orders, "minimum_score", "minimum_score", nil)
