...
const (
	// identifierMarkerPrefix and its companions implement dialect-independent SQL
	// generation. At Build() time the target database is unknown, so every identifier
	// (table and column names) is wrapped in these markers and only quoted for the
	// concrete dialect by renderSQLForDialect right before execution.
...
// rawIdentifier wraps an identifier in the internal TSQ marker.
...
func renderSQLForExecutor(exec Executor, raw string) string {
...
func dialectForExecutor(exec Executor) tsqdialect.Dialect {
	switch tx := exec.(type) {
